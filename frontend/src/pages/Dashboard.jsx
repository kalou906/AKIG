...
import EndpointStatusGrid from '../components/genius/EndpointStatusGrid';
import AIInsights from '../components/ai/AIInsights';

export default function Dashboard() {
  const navigate = useNavigate();
  const [loading, setLoading] = useState(true);
  const { geniusEnabled, density, uiMode } = useUIConfig();
  ...
  return (
    <MainLayout>
      {/* Header */}
      <div className={`${density === 'compact' ? 'mb-4' : 'mb-8'}`}>
        <div className="flex items-center justify-between">
          <div>
            <h1 className={`font-bold ${geniusEnabled ? 'text-white' : 'text-gray-900'} ${density === 'compact' ? 'text-2xl' : 'text-3xl'}`}>
              Bienvenue, {user.name || 'Utilisateur'} 👋
            </h1>
            <p className={`${geniusEnabled ? 'text-white/80' : 'text-gray-600'} ${density === 'compact' ? 'mt-1 text-sm' : 'mt-2'}`}>
              Voici un aperçu de votre activité aujourd'hui
            </p>
          </div>
          <button
            onClick={() => loadDashboardData()}
            className={`px-4 ${density === 'compact' ? 'py-1.5 text-sm' : 'py-2'} ${geniusEnabled ? 'bg-white/20 text-white hover:bg-white/30' : 'bg-blue-600 text-white hover:bg-blue-700'} rounded-lg transition-colors flex items-center gap-2`}
          >
            <BarChart3 size={18} />
            Actualiser
          </button>
      ...
      {/* Alertes */}
      {alerts.length > 0 && (
        <div className={`space-y-3 ${density === 'compact' ? 'mb-4' : 'mb-6'}`}>
          {alerts.map(alert => (
            <div
              key={alert.id}
              className={`${density === 'compact' ? 'p-3' : 'p-4'} rounded-lg border flex items-center gap-3 ${alert.severity === 'warning'
                ? geniusEnabled ? 'bg-yellow-500/20 border-yellow-400/30 text-white' : 'bg-yellow-50 border-yellow-200'
                : geniusEnabled ? 'bg-blue-500/20 border-blue-400/30 text-white' : 'bg-blue-50 border-blue-200'
                }`}
            >
              <Bell className={alert.severity === 'warning' ? 'text-yellow-600' : 'text-blue-600'} size={20} />
              <span className={`${density === 'compact' ? 'text-xs' : 'text-sm'} font-medium ${geniusEnabled ? 'text-white' : 'text-gray-800'}`}>{alert.message}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div>
          ...
          {/* AI Insights (Pro mode) */}
          {(uiMode === 'pro' || uiMode === 'modern') && (
      ...
      {/* API System Overview (Pro mode) */}
      {uiMode === 'pro' && (
        <div className="mt-8">
          <h2 className={`text-xl font-bold ${geniusEnabled ? 'text-white' : 'text-gray-900'} mb-4`}>Aperçu Système API</h2>
          <EndpointStatusGrid />
        </div>
      )}
    </MainLayout>
